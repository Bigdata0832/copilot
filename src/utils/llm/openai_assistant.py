...
def create_message_in_thread(thread_id, role, content):
    """
...
if __name__ == "__main__":
    ...
    # Retrieve Assistant
    assistant = retrieve_assistant("拿破崙")
    print(assistant.id)
    ...
    # Create Thread
    thread = create_thread(messages=[{"role": "user", "content": "Hello"}])
    print(thread.id)
    ...
    create_message_in_thread(thread_id=thread.id, role="user", content="你誰啊")
    ...
    # Get Chat History
    chat_history = get_chat_history(run=run, thread_id=thread.id)
    print(chat_history)
